...
compile
    :: Html a -> TOC.TableOfContents -> a -> Either ManualError LBS.ByteString
...
blockTag blockRule = do
    ...
    sectionRule <-
        C.tag "section" "A container which deliniates sections of a page." $ do
            ...
            pure $ do
                let theSlug =
                        slug 0 $ title `fromMaybe` tocTitle `fromMaybe` slugText
...
hexadecimalize = go []
  where
    go []  0 = "0"
    go acc 0 = acc
    go acc n | n >= 16   = let (q, r) = quotRem n 16 in go (go acc r) q
...
toc theToc = H.ol $ do
    ...
    where (before, (thisSlug, thisContents), after) = TOC.orderedEntries theToc
...
tocCurrent fp slug tocEntry = H.li ! HA.class_ "current" $ do
    H.a (H.text $ TOC.entryTitle tocEntry)
        ...
        ! HA.href
              (  H.toValue (fileSlugPath fp)
              <> foldMap (("#" <>) . H.toValue) slug
              )
    H.ol
        . for_ (TOC.toList $ TOC.entryChildren tocEntry)
        $ \(slug, children) -> tocCurrent fp (Just slug) children

tocOther :: FileSlug -> Metadata -> H.Html
tocOther fp md | md ^. property "hidden" = mempty
