...
instance MFunctor (ProductRule input) where
    hoist f (ProductRule md eval) =
        ProductRule md $ \input -> Ap . f . getAp $ eval input
...
instance MFunctor (SumRule input) where
    hoist f (SumRule md eval) =
        SumRule md $ \input -> Ap . f . getAp <$> eval input

instance Functor context => Functor (SumRule input context) where
    fmap f (SumRule m e) = SumRule m $ fmap (fmap (fmap f)) . e

annotateProduct
    :: (P.FromSource input, Functor context)
    => ProductRule input context output
    -> ProductRule input context output
annotateProduct ~(ProductRule meta eval) = ProductRule meta $ \input ->
    Error.mapErrors
            (Eval.WrappedEvalError (meta ^. field @"key") (input ^? P.location))
        <$> eval input
...
annotateSum ~(SumRule meta eval) = SumRule meta $ \input ->
    fmap
            (Error.mapErrors
                (Eval.WrappedEvalError (meta ^. field @"key")
                                       (input ^? P.location)
                )
            )
        <$> eval input
...
compileM
    ...
    -> input
    -> context (Either CompileError output)
...
chooseOne thisKey description choices = liftCompileM $ do
    let subRuleKeys =
            HashSet.fromList
                ...
                .   field @"key"
        thisProductInfo =
            Info.ProductInfo thisKey description mempty mempty subRuleKeys
        thisInfo = Info.Product thisProductInfo
    Util.setOnceFail
        (L.at thisKey)
        (SpecError . Error.singleton $ Spec.RuleConflict thisKey)
...
disallow
    :: forall input output context
     . (Typeable input, Applicative context)
    => Text
    -> ProductRule input context output
disallow message = ProductRule
    { metadata = Info.ProductInfo { key = Info.infoKey @input "Disallowed"
...
descendWith
    :: (P.FromSource input, Monad context)
    => Descent context output input child
    -> ProductRule child context output
    -> Desc input context output
descendWith sel ~(ProductRule md eval) = Desc $ do
    pure . Eval.Eval $ \input state ->
        let result = getAp $ input ^. sel . L.to eval
            mapErr = Error.mapErrors
                (Eval.WrappedEvalError (md ^. field @"key")
                                       (input ^? P.location)
                )
        in  fmap ((, state) . mapErr) result
